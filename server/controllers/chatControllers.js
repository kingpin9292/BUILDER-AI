...
export async function chat(req, res) {
  ...
  try {
    ...
    //Include All files contents so the AI cand dot accurate search/replace
    ...
    console.log(`[AI] Got ${result.operations.length} operation:${result.description}`);

    //Apply opeartaions to file map

    const { files: updatedFiles, applied, errors } = applyOperations(project.files, result.operations);

    if (errors.length > 0) {
      console.warn(`[Diff] Errors applying opeartions:`, errors);
    }
    ...
    project.messages.push({
      role: "assistant",
      content: result.description + (error.length > 0 ? `\n\n Some operation failed:${errors.join(", ")}` : ""),
    });
    await project.save();
