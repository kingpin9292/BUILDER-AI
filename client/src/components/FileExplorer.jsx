...
function getFileIcon(fileName) {
  if (fileName.endsWith(".css")) return <FileTextIcon size={14} className="text-sky-500" />;
  if (fileName.endsWith(".js") || fileName.endsWith(".jsx"))
    return <FileCodeIcon size={14} className="text-amber-500" />;
  if (fileName.endsWith(".json")) return <FileTextIcon size={14} className="text-emerald-500" />;
...
function TreeItem({ node, activeFile, onFileSelect, depth = 0 }) {
  ...
  return (
    <button
      ...
      className={`w-full flex item-center gap-2 py-1.5 px-2 text-xs transition-colors rounded-md cursor-pointer ${isActive ? "bg-zinc-100 text-zinc-950 font-medium" : "text-zinc-500 hover:bg-zinc-50 hover:text-zinc-900"}`}
      style={{ paddingLeft: `${depth * 12 + 8}px` }}
    >
